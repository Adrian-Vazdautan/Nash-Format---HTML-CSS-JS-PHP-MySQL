		...
		function saveData($article_title, $article_text, $article_thumbnail_des, $article_category, $article_notifications){
				#START : Read time
					#Article text: $article_text
						$read_time_ARTICLE_TEXT = $article_text;
					#Пример переменной с HTML-кодом, включая код изображения
						$read_time_ARTICLE_TEXT = $read_time_ARTICLE_TEXT;
					#Удаление HTML-тегов и их содержимого
						$clean_text = preg_replace('/<[^>]*>/', ' ', $read_time_ARTICLE_TEXT);
			...
			#INCLUDE
			    ...
			    #Prepare the statement
			        $query = "INSERT INTO articles100percent (code, nickname, category, title, textarea, reading_time, thumbnail_description, allow_notifications, status) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)";
			        if($stmt = mysqli_prepare($connect, $query)) {
			        		$status = 0;
			            #Bind the parameters
			                mysqli_stmt_bind_param($stmt, "ssssssssi", $code, $save_article_session_nickname, $article_category, $article_title, $article_text, $reading_time_minutes, $article_thumbnail_des, $article_notifications, $status);
