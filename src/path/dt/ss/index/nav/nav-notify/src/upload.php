                    ...
                    if(mysqli_num_rows($query_upload_notify) == 0){
                        ...
                        while($notify = mysqli_fetch_assoc($query_upload_notify)){
                            #START : Query ->get title from article
                                #Query
                                    $query_get_title_FA = mysqli_query($connect, "SELECT title FROM articles WHERE id='".$notify['id_of_article']."'");
                                            ...
                                            #Check if image is stored in BD
                                                    ...
                                                    #START : Obtain image_name.png from bd
                                                        ...
                                                        #Check if query was successfull
                                                            if($query_image_name){
                                                                #Запрос выполнен успешно 
                                                                    $Show_Avatar = "<img loading='lazy' class='l w36 h36 br50' src='src/du/avatar/". $query_image_name[0] ."'/>";
                                                            } else {
                                                                #В случае ошибки
                                                            }
                                                    #END
