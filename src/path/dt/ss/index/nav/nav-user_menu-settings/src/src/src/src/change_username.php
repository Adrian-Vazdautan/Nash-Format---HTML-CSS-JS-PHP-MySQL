		...
		function update_username_in_table_users($connect, $new_username, $userWhichSignIn){
		    ...
		    #Check
		        if($query_update_username){
		            #SUCCESS
		            	update_username_in_table_articles($connect, $new_username, $userWhichSignIn);
		        } else {
		            echo "Error: " . mysqli_error($connect); #Ошибка при выполнении запроса
		        }
		}
	#END
	#START : 02 articles: UPDATE username/nickname in table : articles
		function update_username_in_table_articles($connect, $new_username, $userWhichSignIn){
			#Query check if username exist
		        $query_update_username = mysqli_query($connect, "UPDATE articles SET nickname='$new_username' WHERE nickname='$userWhichSignIn'");
		    #Check
		        if($query_update_username){
		            #SUCCESS
