<?php
	...
	if(!isset($_SESSION["user"])){
		...
		#START ARTICLES
			#DUPLICATE 002
				if(strpos($_SERVER['REQUEST_URI'], "/feed.php") !== false || strpos($_SERVER['REQUEST_URI'], "/article.php") !== false || strpos($_SERVER['REQUEST_URI'], "/profile.php") !== false || strpos($_SERVER['REQUEST_URI'], "/add.php") !== false){
					echo "<!--START ARTICLES--><div class='id_index_articles_session_true_js APPEARANCE_iiasfj_js DARK_THEME_COLOR_2 a p l'>";
		...
		#START USER PROFILE
			#DUPLICATE 003
				if(strpos($_SERVER['REQUEST_URI'], "/feed.php") !== false || strpos($_SERVER['REQUEST_URI'], "/article.php") !== false || strpos($_SERVER['REQUEST_URI'], "/profile.php") !== false || strpos($_SERVER['REQUEST_URI'], "/add.php") !== false){
					echo "<!--START MY PROFILE--><div class='mwupwsnjs p l none'>";
...
<script>
	//System messages
		var field_is_required             = "Отвечай",
			minimum_length_5_characters   = "Больше 5 символов",
			email_is_already_used         = "Почта занята уже",
			this_username_is_already_used = "Придумай другое имя";
</script>
